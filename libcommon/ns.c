...
bool nsEnter(uintptr_t cloneFlags)
{
    ...
    if (files_writeBufToFile("/proc/self/setgroups", (const uint8_t*)deny_str, strlen(deny_str), O_WRONLY) == false) {
    ...
    if (files_writeBufToFile("/proc/self/gid_map", (const uint8_t*)gid_map, strlen(gid_map), O_WRONLY) == false) {
    ...
    if (files_writeBufToFile("/proc/self/uid_map", (const uint8_t*)uid_map, strlen(uid_map), O_WRONLY) == false) {
